...
namespace WebHelper
{
    public partial class IWebHelper
    {
        ...
        public string m_chr_extension_dir = Path.Combine(Program.g_working_directory, "ChromeExtension");
        ...
        public async Task<bool> Start()
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!System.IO.File.Exists(chr_path))
                    {
                        MyLogger.Error($"#{m_ID} - chrome.exe Not found. Perhaps the Google Chrome browser is not installed on this computer.");
                        return false;
                    }
                    ...
                    {
                        MyLogger.Error($"#{m_ID} - Fail to start chrome.exe. Please make sure any other chrome windows are not opened.\n{ex.Message}");
                        return false;
                    }
                    ...
                    //MyLogger.Info($"#{m_ID} - Browser successfully started.");
                    return true;
                }
                catch (Exception exception)
                {
                    MyLogger.Error($"#{m_ID} - Failed to start. Exception:{exception.Message}\n{exception.StackTrace}");
                    try
                    {
                        WebDriver.Quit();
                    }
                    ...
                    {
                        MyLogger.Error($"#{m_ID} - Failed to quit driver. Exception:{ex.Message}");
                    }
                    return false;
                }
            }
            ...
            {
                MyLogger.Error($"#{m_ID} - Exception occured while trying to start chrome driver. Exception:{exception.Message}");
            }
            return false;
        }
