...
namespace WebAuto
{
    public class KWebGetegiftcard : KWebBase
    {
        public KWebGetegiftcard() : base()
        {
        ...
        {
            if (link.StartsWith("https://getegiftcard.walmart.com/"))
                return true;
        ...
        protected async Task<int> scrap(string link, string mailto, List<ZGiftCardDetails_V2> card_details)
        {
            ...
            try
            {
                ...
                MyLogger.Info("Getegiftcard browser started successfully.");
                ...
                string strLogoXpath = "//div[@id='cert-preview']//img";
                string strLogoTxt = await get_value(strLogoXpath, "", "longdesc");
                int nIdxOrder = strLogoTxt.IndexOf("order number") + "order number".Length;
                temp = strLogoTxt.Substring(nIdxOrder).Replace(" ", "");
                string strOrder = temp.Substring(0, temp.IndexOf("."));
                MyLogger.Info($"Order - {strOrder}");
